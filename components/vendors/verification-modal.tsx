...
import { useVerify } from "@/hooks/useVendors";
import { toast } from "@/hooks/use-toast";

export function RejectVerificationDialog({ vendorId }: { vendorId: string }) {
	const [reason, setReason] = useState("");
	const [isOpen, setIsOpen] = useState(false);

	const { mutateAsync: rejectVerification, isPending } = useVerify({
		vendorId,
	});

	const handleReject = async ({ newStatus }: { newStatus: string }) => {
		try {
			await rejectVerification({ vendorId, newStatus, reason });
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent className="sm:max-w-[425px]">
				<DialogHeader>
					...
					<DialogDescription>
						Please provide a reason for rejecting this vendor's verification
						request. This reason will be sent to the vendor.
					</DialogDescription>
				</DialogHeader>
